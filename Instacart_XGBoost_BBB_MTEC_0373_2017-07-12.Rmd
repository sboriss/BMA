...
##Add_on:
- 2017-07-13: add order_dow (day_of_week) and order_hour_of_day to feature set in the model
...
### Add 
```{r}
orders_tmp = head( orders, n = 11 )
orders_test_tmp = orders %>% filter( eval_set == 'test' ) %>% 
                         head( n = 10 )
ordert_tmp = head( ordert, n = 10 )
orderp_tmp = head( orderp, n = 10 )
```
...
- rows with eval_set = 'train' and eval_set = 'test' are gone
```{r}
...
### check what happens to test rows in orders: these are not there
```{r}
orders_products_test_tmp = orders_products %>% 
...
```{r}
prd_tmp0 <- orders_products %>% filter( user_id == 1) %>% 
...
- prod_reorder_times:
  - aka prd\$prod\_orders / prd\$prod\_first\_orders as 
...
- user_mean_order_dow        : average dow for ordering
- user_mean_order_hour_of_day: average hod for ordering
```{r}
# Users -------------------------------------------------------------------
users <- orders %>%
  ...
  summarise(
    ...
    user_mean_order_hour_of_day = mean( order_hour_of_day )
  )
...
```{r}
# Users -------------------------------------------------------------------
hist( users$user_mean_order_dow)
hist( users$user_mean_order_hour_of_day )
```
...
us <- orders %>%
  ...
  select(user_id, order_id, eval_set,
         days_since_last_prior_order = days_since_prior_order, 
         postprior_order_dow         = order_dow, 
         postprior_order_hour_of_day = order_hour_of_day
         )
